...
dash.register_page(__name__, name='오늘의 비트바타 날씨', title='BITmaster')
...
layout = dbc.Container([
    # title
    dbc.Row([
        dbc.Col([html.H3(['비트바타가 알려주는 날씨'])], width=12, className='row-titles')
    ]),
    # Transformations
    dbc.Row([
        dbc.Col([], width = 2),
...
def data_transform(_logtr, _d1check, _d1v, _d2check, _d2v, _d3check, _d3v):
    ...
    _hovertemplate = (
        ...
        "<extra></extra>")
    fig_2.add_trace(go.Scatter(x=_data['rolling_avg'], y=_data['rolling_std'], mode='markers', marker_size=4, marker_color='#ffc107', hovertemplate=_hovertemplate))
